...
export function matchSkills(jdSkills: string[], userSkills: string[]): SkillMatch {
  const normalizedUser = normalizeSkills(userSkills);
  const normalizedJD = jdSkills.map((s) => s.toLowerCase());

  const matched = normalizedJD.filter((skill) => normalizedUser.some((us) => isMatch(skill, us)));
  const missing = normalizedJD.filter((skill) => !normalizedUser.some((us) => isMatch(skill, us)));

  const percentage = normalizedJD.length > 0 ? Math.round((matched.length / normalizedJD.length) * 100) : 0;
